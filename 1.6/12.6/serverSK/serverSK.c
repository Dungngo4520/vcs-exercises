...
// create data structure to modify server thread agruments
#define FAILED -1
#define SUCCESS 0
typedef struct {
	SERVER_ARG** servers;
...
//functions work with SERVER_LIST structure

int ServerListResize(SERVER_LIST* s, size_t size) {
	int status = FAILED;
	if (s) {
		SERVER_ARG** servers = realloc(s->servers, sizeof(SERVER_ARG*) * size);
		if (servers) {
			s->servers = servers;
			s->size = size;
			status = SUCCESS;
...
int ServerListAdd(SERVER_LIST* s, void* sTh) {
	int status = FAILED;
	if (s) {
		ServerListResize(s, s->size + 1);
		s->servers[s->size] = sTh;
		status = SUCCESS;
	}
	return status;
}

int ServerListDelete(SERVER_LIST* s, int index) {
	int status = FAILED;
	int i = 0;
	...
	if (s) {
		...
		CloseHandle(s->servers[index]->hSrvThread);
		s->servers[index] = NULL;
		...
		free(s->servers[s->size - 1]);
		s->size--;
		ServerListResize(s, s->size);
		status = SUCCESS;

	}
	return status;
}
int ServerListDeleteNumber(SERVER_LIST* s, int number) {
	int status = FAILED;

	if (s) {
		for (int i = 0; i < s->size; i++) {
			if (s->servers[i]->number == number) {
				CloseHandle(s->servers[i]->hSrvThread);
				s->servers[i] = NULL;
...
int ServerThreadTerminatedPos(SERVER_LIST* s) {
	int status = FAILED;
	for (int i = 0; i < s->size; i++) {
		BOOL isStopped = FALSE;
		EnterCriticalSection(&s->servers[i]->threadCs);
		__try {

			if (s->servers[i]->thState == SERVER_THREAD_STOPPED) {
				int tStatus = WaitForSingleObject(s->servers[i]->hSrvThread, INFINITE);
		...
		__finally {
			...
			LeaveCriticalSection(&s->servers[i]->threadCs);
		}
...
static SERVER_LIST svrList = { NULL,0 };
...
int _tmain(int argc, LPCTSTR argv[]) {
	...
	while (!shutFlag) {
		...
		hAcceptThread = (HANDLE)_beginthreadex(NULL, 0, AcceptThread, NULL, 0, NULL);
	...
	while (TRUE) {
		...
		for (iThread = 0; iThread < svrList.size; iThread++) {
			EnterCriticalSection(&svrList.servers[iThread]->threadCs);
			__try {
				if (svrList.servers[iThread]->thState == SERVER_THREAD_RUNNING ||
					svrList.servers[iThread]->thState == SERVER_THREAD_STOPPED) {
					if (WaitForSingleObject(svrList.servers[iThread]->hSrvThread, 10000) == WAIT_OBJECT_0) {
						_tprintf(_T("Server thread on slot %d stopped.\n"), iThread);
						CloseHandle(svrList.servers[iThread]->hSrvThread);
						svrList.servers[iThread]->hSrvThread = NULL;
						svrList.servers[iThread]->thState = SERVER_SLOT_INVALID;
					}
					else if (WaitForSingleObject(svrList.servers[iThread]->hSrvThread, 10000) == WAIT_TIMEOUT) {
			...
			__finally { LeaveCriticalSection(&svrList.servers[iThread]->threadCs); }
		}
...
static DWORD WINAPI AcceptThread() {
	LONG addrLen;
	SERVER_ARG* sArg = malloc(sizeof(SERVER_ARG));
	if (!sArg)ReportError(_T("Failed allocating server thread"), 1, TRUE);

	// create new thread and add to list
	//Intialize thread arg
	InitializeCriticalSection(&sArg->threadCs);
	sArg->number = svrList.size;
	sArg->thState = SERVER_SLOT_FREE;
	sArg->hDll = svrList.hDll;
	sArg->hSrvThread = NULL;

	addrLen = sizeof(connectSAddr);
	sArg->sock =
		accept(SrvSock, (struct sockaddr*)&connectSAddr, &addrLen);
	if (sArg->sock == INVALID_SOCKET) {
		ReportError(_T("accept: invalid socket error"), 1, TRUE);
		return 1;
	}

	ServerListAdd(&svrList, sArg);

	/* A new connection. Create a server thread */
	EnterCriticalSection(&(sArg->threadCs));
	__try {
		sArg->hSrvThread = (HANDLE)_beginthreadex(NULL, 0, Server, sArg, 0, NULL);
		if (sArg->hSrvThread == NULL)
			ReportError(_T("Failed creating server thread"), 1, TRUE);
		sArg->thState = SERVER_THREAD_RUNNING;
		_tprintf(_T("Client accepted on slot: %d, using server thread %d.\n"), sArg->number, GetThreadId(sArg->hSrvThread));
		/* Exercise: Display client machine and process information */
	}
	__finally { LeaveCriticalSection(&(sArg->threadCs)); }
	return 0;
}
...
/* Server thread function. There is a thread for every potential client. */
{
	...
	// detele teminated server threads ////
	ServerListDeleteNumber(&svrList, pThArg->number);
...
BOOL ReceiveRequestMessage(REQUEST* pRequest, SOCKET sd) {
	...
	while (nRemainRecv > 0 && !disconnect) {
		...
		if (nXfer == SOCKET_ERROR) {
			_tprintf(_T("recv error code %d, "), WSAGetLastError());
...
BOOL SendResponseMessage(RESPONSE* pResponse, SOCKET sd) {
	...
	while (nRemainSend > 0 && !disconnect) {
		...
		if (nXfer == SOCKET_ERROR) {
			_tprintf(_T("recv error code %d, "), WSAGetLastError());
			ReportError(_T("server send() failed"), 13, TRUE);
		}
		disconnect = (nXfer == 0);
		nRemainSend -= nXfer;
		pBuffer += nXfer;
	...
	while (nRemainSend > 0 && !disconnect) {
		...
		if (nXfer == SOCKET_ERROR) {
			_tprintf(_T("recv error code %d, "), WSAGetLastError());
			ReportError(_T("server send() failed"), 14, TRUE);
		}
		disconnect = (nXfer == 0);
		nRemainSend -= nXfer;
		pBuffer += nXfer;
