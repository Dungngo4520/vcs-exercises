...
/*grepMPExtended
	SYNOPSIS
		grepMPExtended [OPTIONS] PATTERNS [FILES]
	OPTIONS
		-s
			Suppresses error report when a file does not exist
		-i
	...
	in the current directory, to receive the results. */
{
	...
	int iProc, iPattern;
	BOOL dashS = FALSE, ignoreCase = FALSE;
	...
	iPattern = Options(argc, argv, _T("si"), &dashS, &ignoreCase, NULL);
	...
	for (iProc = 0; iProc < argc - iPattern - 1; iProc++) {

		/* Create a command line of the form: grep argv[1] argv[iProc + 2] */
		/* Allow spaces in the file names. */
		if (ignoreCase) {
			_stprintf_s(commandLine, sizeof(commandLine), _T("grep -i \"%s\" \"%s\""), argv[iPattern],
			            argv[iProc + iPattern + 1]);
		}
		else {
			_stprintf_s(commandLine, sizeof(commandLine), _T("grep \"%s\" \"%s\""), argv[iPattern],
			            argv[iProc + iPattern + 1]);
		}
	...
	for (iProc = 0; iProc < argc - iPattern - 1; iProc += MAXIMUM_WAIT_OBJECTS)
		WaitForMultipleObjects(min(MAXIMUM_WAIT_OBJECTS, argc - iPattern-1 - iProc),
	...
	for (iProc = 0; iProc < argc - iPattern - 1; iProc++) {
		if (GetExitCodeProcess(hProc[iProc], &exitCode) && exitCode == 0) {
			...
			if (dashS) {
				_stprintf_s(commandLine, sizeof(commandLine), _T("cat -s \"%s\""), procFile[iProc].tempFile);
			}
			else {
				_stprintf_s(commandLine, sizeof(commandLine), _T("cat \"%s\""), procFile[iProc].tempFile);
			}
