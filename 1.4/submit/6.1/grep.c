...
prepSearchString(char* p, char* buf) {
	...
	for (;;) {
		switch (c = *p++) {
		case EOS: goto Exit;
		case '*': if (i >= 0 && buf[i - 1] != STAR) c = STAR;
			break;
		case '?': c = QM;
			break;
		case '[': c = BEGCLASS;
...
Only the pattern should be on the command line. */
{
	...
	iPattern = Options(argc, argv, _T("i"), &ignoreCase, NULL);

	for (iFile = iPattern + 1; iFile < argc; iFile++) {
		...
		for (i = iPattern; i < argc + 1; ++i) {
			if (!patternSeen++)
				...
				while (fgets(string, sizeof (string), fp) != NULL
					&& !feof(fp)) {
					char temp[2048];
					_tcscpy_s(temp, sizeof(temp), string);
					if (ignoreCase) _strlwr_s(temp, sizeof(temp));
					if (patternMatch(pattern, temp)) {
						result = 0;
						fputs(string, stdout);
					}
				}
