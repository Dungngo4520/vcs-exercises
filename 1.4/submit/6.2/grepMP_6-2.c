	...
	in the current directory, to receive the results. */
{
	...
	for (iProc = 0; iProc < argc - 2; iProc++) {
		...
		// There is no "grepHA.c" file, and grep code does not use the windows API function, so I leave it untouched.
	...
	for (iProc = 0; iProc < argc - 2; iProc++) {
		if (GetExitCodeProcess(hProc[iProc], &exitCode) && exitCode == 0) {
			...
			// Convert HANDLE to DWORD, then to character string
			...
			TCHAR handleToString[11];
			_itoa_s((DWORD)hTempFile, handleToString, sizeof(handleToString), 10);
			CloseHandle(hTempFile);

			_stprintf(commandLine, _T("catHA \"%s\""), handleToString);
			if (!CreateProcess(NULL, commandLine, NULL, NULL,
