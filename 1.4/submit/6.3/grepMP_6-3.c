	...
	in the current directory, to receive the results. */
{
	...
	DWORD exitCode, dwCreationFlags = 0, iProcSignaled;
	int iProc;
	...
	for (iProc = 0; iProc < argc - 2; iProc++) {
		// Wait for any process to terminate, the number of process to wait decreases each loop

		iProcSignaled = WaitForMultipleObjects(min(MAXIMUM_WAIT_OBJECTS, argc - 2 - iProc), hProc, FALSE, INFINITE)
			- WAIT_OBJECT_0;

		/* Result files sent to std output using "cat".
		Delete each temporary file upon completion. */

		if (GetExitCodeProcess(hProc[iProcSignaled], &exitCode) && exitCode == 0) {
			/* Pattern was detected - List results. */
			/* List the file name if there is more than one file to search */
			if (argc > 3)
				_tprintf(_T("%s:\n"), argv[iProcSignaled + 2]);
			_stprintf(commandLine, _T("cat \"%s\""), procFile[iProcSignaled].tempFile);
			if (!CreateProcess(NULL, commandLine, NULL, NULL,
		...
		CloseHandle(hProc[iProcSignaled]);
		if (!DeleteFile(procFile[iProcSignaled].tempFile))
			ReportError(_T("Cannot delete temp file."), 6, TRUE);

		// Place the terminated process' HANDLE and PROCFILE to the end of its array,
		// and shift the array to the left by 1

		HANDLE temp = hProc[iProcSignaled];
		PROCFILE tempProcFile = procFile[iProcSignaled];
		for (int i = iProcSignaled; i < argc - 3; i++) {
			hProc[i] = hProc[i + 1];
			procFile[i] = procFile[i + 1];
		}
		hProc[argc - 3] = temp;
		procFile[argc - 3] = tempProcFile;
	}
