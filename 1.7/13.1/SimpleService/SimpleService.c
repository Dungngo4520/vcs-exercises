...
VOID LogEvent(LPCTSTR UserMessage, WORD type)
{
	...
	//fflush(logFp);+
	LPCSTR messages[1] = { UserMessage };

	WORD wNumStrings = 1;
	DWORD dwDataSize = 0;
	LPCSTR *lpStrings = messages;
	LPVOID lpRawData = NULL;

	if (type = EVENTLOG_SUCCESS) {
		if (!ReportEvent(hEventLog, EVENTLOG_SUCCESS, 0, 0, NULL, wNumStrings, dwDataSize, lpStrings, lpRawData)) {
			ReportError(_T("ReportEvent failed"), 0, TRUE);
		}
	}
	else if (type = EVENTLOG_INFORMATION_TYPE) {
		if (!ReportEvent(hEventLog, EVENTLOG_INFORMATION_TYPE, 0, 0, NULL, wNumStrings, dwDataSize, lpStrings, lpRawData)) {
			ReportError(_T("ReportEvent failed"), 0, TRUE);
		}
	}
	else if (type = EVENTLOG_ERROR_TYPE) {
		if (!ReportEvent(hEventLog, EVENTLOG_ERROR_TYPE, 0, 0, NULL, wNumStrings, dwDataSize, lpStrings, lpRawData)) {
			ReportError(_T("ReportEvent failed"), 0, TRUE);
		}
	}
	else if (type = EVENTLOG_WARNING_TYPE) {
		if (!ReportEvent(hEventLog, EVENTLOG_WARNING_TYPE, 0, 0, NULL, wNumStrings, dwDataSize, lpStrings, lpRawData)) {
			ReportError(_T("ReportEvent failed"), 0, TRUE);
		}
	}
	else if (type = EVENTLOG_AUDIT_FAILURE) {
		if (!ReportEvent(hEventLog, EVENTLOG_AUDIT_FAILURE, 0, 0, NULL, wNumStrings, dwDataSize, lpStrings, lpRawData)) {
			ReportError(_T("ReportEvent failed"), 0, TRUE);
		}
	}
	else if (type = EVENTLOG_AUDIT_SUCCESS) {
		if (!ReportEvent(hEventLog, EVENTLOG_AUDIT_SUCCESS, 0, 0, NULL, wNumStrings, dwDataSize, lpStrings, lpRawData)) {
			ReportError(_T("ReportEvent failed"), 0, TRUE);
		}
	}
	return;
...
BOOL LogInit(LPTSTR name)
{
	...
	//return (logFp != NULL);
	hEventLog = RegisterEventSource(NULL, _T("SimpleService EventLog"));
	if (hEventLog != NULL)LogEvent(_T("Initialized Logging"), EVENTLOG_SUCCESS);
