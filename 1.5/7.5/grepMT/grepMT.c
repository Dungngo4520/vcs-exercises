	...
	This program modifies Program 8-1, which used processes. */
{
	...
	FILETIME fCreationTime, fExitTime, fKernelTime, fUserTime, fThreadTime;
	ULARGE_INTEGER uCreationTime, uExitTime, timeElapsed;
	SYSTEMTIME sThreadTime;
	...
	threadCount = argc - 2;
	for (iThrd = 0; iThrd < threadCount; iThrd += MAXIMUM_WAIT_OBJECTS) {
		threadIndex = WaitForMultipleObjects(min(MAXIMUM_WAIT_OBJECTS, threadCount - iThrd), tHandle, TRUE, INFINITE)
			- (int)WAIT_OBJECT_0;
		if (threadIndex < 0 || threadIndex >= threadCount)
			ReportError(_T("Thread wait error."), 5,TRUE);
	}

	for (iThrd = 0; iThrd < threadCount; iThrd ++) {
		GetExitCodeThread(tHandle[iThrd], &exitCode);
		if (!GetThreadTimes(tHandle[iThrd], &fCreationTime, &fExitTime, &fKernelTime, &fUserTime)) {
			ReportError(_T("Get thread time error."), 6,TRUE);
		}
		CloseHandle(tHandle[iThrd]);

		// Calculate time different
		uCreationTime.LowPart = fCreationTime.dwLowDateTime;
		uCreationTime.HighPart = fCreationTime.dwHighDateTime;
		uExitTime.LowPart = fExitTime.dwLowDateTime;
		uExitTime.HighPart = fExitTime.dwHighDateTime;
		timeElapsed.QuadPart = uExitTime.QuadPart - uCreationTime.QuadPart;
		...
		if (exitCode == 0) {
			...
			_tprintf(_T("Thread time: %.3fms\n\n"), timeElapsed.QuadPart / 10000.0);
