...
unsigned WINAPI Produce(void*);
unsigned WINAPI Consume(void*);
...
int _tmain(int argc, LPTSTR argv[]) {
	...
	if (mBlock.mGuard == NULL) {
		ReportError(_T("Cannot create mutex"), 5, TRUE);
	}
...
unsigned WINAPI Produce(void* arg)
/* Producer thread - Create new messages at random intervals */
{
	...
	while (!mBlock.fStop) {
		...
		dwWaitResult = WaitForSingleObject(mBlock.mGuard, INFINITE);
		switch (dwWaitResult) {
		case WAIT_OBJECT_0:
			__try {
				if (!mBlock.fStop) {
...
unsigned WINAPI Consume(void* arg) {
	CHAR command, extra;
	DWORD dwWaitResult;
