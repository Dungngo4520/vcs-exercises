...
int _tmain(int argc, LPTSTR argv[]) {
	...
	if (!noPrint) {
		...
		while (ReadFile(hFile, buffer, BUF_SIZE, &nIn, NULL) && (nIn != 0) &&
...
DWORD WINAPI SortThread(PTHREADARG pThArg) {
	...
	pRecord = MapViewOfFile(mHandle, FILE_MAP_ALL_ACCESS, 0, 0, 0);
	if (NULL == pRecord)
		ReportError(_T("Failure to map input file."), 6, TRUE);
	CloseHandle(mHandle);

	numbersInGroup = nRec / numThreads;
	threadNumber = pThArg->iTh;
	lowRecNum = nRec / numThreads * threadNumber; //determine the start record position
	highRecNum = lowRecNum + numbersInGroup;
	pThArg->lowRecord = pRecord + lowRecNum;
	pThArg->highRecord = pRecord + lowRecNum + nRec / numThreads;
