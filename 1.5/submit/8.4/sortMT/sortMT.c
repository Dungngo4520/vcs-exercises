...
int _tmain(int argc, LPTSTR argv[]) {
	...
	ghSemaphore = CreateSemaphore(NULL, numThreads, numThreads,NULL);
...
DWORD WINAPI SortThread(PTHREADARG pThArg) {
	...
	LPRECORD first;
	DWORD lowRecNum, highRecNum, nextGroupRec, dwWaitResult;
...
static VOID MergeArrays(LPRECORD p1, LPRECORD p2, DWORD p1Recs, DWORD p2Recs) {
	...
	DWORD iRec = 0, i1 = 0, i2 = 0, totalRec, dwWaitResult;
