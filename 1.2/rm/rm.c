	...
	the low order part of the file size. */
{
	...
	TCHAR filePath[MAX_PATH_LONG + 1];
	if (fType ==TYPE_DOT) return FALSE;
	if (fType == TYPE_DIR) {
		if (!RemoveDirectory(pFileData->cFileName)) {
			_tprintf(_T("Failed to delete folder: %s\n"), pFileData->cFileName);
		}
		else {
	...
	if (fType == TYPE_FILE) {
		if (!DeleteFile(pFileData->cFileName)) {
			_tprintf(_T("Failed to delete file: %s\n"), pFileData->cFileName);
		}
		else {
			_tprintf(_T("Deleted: %s\n"), pFileData->cFileName);
		}
